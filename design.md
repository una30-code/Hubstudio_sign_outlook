...
**表格级契约**（`.env` 键、Hubstudio 字段映射、冻结 `data` 键、`step` 枚举、执行顺序）以 `docs/contracts.md` 为准；本文侧重目标、边界与模块叙事。术语见 `docs/glossary.md`，追溯表见 `docs/codemap.md`。
...
| `apply_signup_profile`    | 录入失败；或人机步骤未启用/未触发时**最后成功步**。                                        |
| `ms_hold_challenge`       | 可选人机验证：失败；或开启且执行长按后**最后成功步**（`success=True`）。                     |
...
- **已实现**：`apply_outlook_signup_profile` 及可选 `ms_hold_challenge` 成功后追加 `logs/archive/phase2_signup_smoke_YYYYMMDD.jsonl`（`append_archive_record`，`phase=phase2_signup_smoke`）；`StepResult.data` 增加 `archive_path`、`archive_ref`。载荷含 `container_code`（若有）、`used_cdp_url_override`、`current_url`、`url_ok` / `element_ok` / `element_hit`、`steps_completed`、`email_used`、`hold_challenge`（摘要）、`verified_at`，**不含密码**。
- 选择器随微软页面改版迭代时，应**单次最小变更** `verify_page.py` / `apply_signup_profile.py` / `ms_hold_challenge.py`，并更新本节说明。

### 10.9 可选：「Press and hold」人机验证（微软）

| 环境变量 | 缺省 | 说明 |
| -------- | ---- | ---- |
| `PHASE2_TRY_HOLD_CHALLENGE` | 关 | 设为 `1` / `true` / `yes` / `on` 时，在 `apply_outlook_signup_profile` 成功后检测页面是否含 **Press and hold**（或简中「按住/长按」）文案；若是则尝试：关闭浏览器「保存密码」条 → 点击 **Accessible / 无障碍** 入口 → 等待 `PHASE2_HOLD_AFTER_ACCESSIBLE_MS` → 对主按钮执行 **长按**（Playwright `click(delay=PHASE2_HOLD_PRESS_DURATION_MS)`）。 |
| `PHASE2_HOLD_AFTER_ACCESSIBLE_MS` | 2500 | 点小人与按长按钮之间的间隔。 |
| `PHASE2_HOLD_PRESS_DURATION_MS` | 4500 | 模拟按住时长（毫秒），可按页面要求加大。 |

**评估与风险（摘要）**

- **可行性**：技术上可用 Playwright 实现「点无障碍入口 + 延时 + 长按」；若 DOM/文案与选择器一致，成功率较高。
- **脆弱性**：微软可随时改版 iframe/DOM/文案；多语言、A/B 版会导致选择器失效，需按截图与 `screenshots/ms_hold_challenge.png` 迭代。
- **遮挡**：`ms_hold_challenge` 会在人机步骤前 **多轮** 尝试关闭「保存密码」（`button:has-text`、role、`force` 点击、对话框第二按钮、连续 Escape）。若弹窗为 **纯浏览器外壳**（不在页面 DOM），请在 Hubstudio/Chromium **设置中关闭「提示保存密码」**，否则脚本无法点击到该层。
- **iframe**：验证区若在子 frame，逻辑会扫描 `page.frames` 并在命中的 frame 内点小人与长按。
- **合规**：请自行确认自动化行为与 Microsoft 服务条款及当地法律；本仓库仅提供可开关的辅助步骤，**不保证**通过风控或后续验证码。

实现：`src/ms_hold_challenge.py` → `try_ms_accessible_hold_challenge`；成功且非跳过时流水线最终 `step` 为 `ms_hold_challenge`。
...
| 2026-04-07     | §10.9：可选 `ms_hold_challenge`（无障碍入口 + Press and hold）；`PHASE2_TRY_HOLD_CHALLENGE` 等                           |
