...
| `PAGE_LOAD_TIMEOUT_MS`                    | 否                    | 导航与部分等待超时（毫秒）。                                                                                                              |
...
| `verify_page`             | 最后一跳；成功时 `success=True`。                                                          |

失败时：`error` 为非空字符串；`open_signup_page` / `verify_page` 在可能时写入 `screenshot_path`（`screenshots/` 下 PNG）。**注意**：`hubstudio_browser_start` / `connect_browser` 失败时通常**无**可用 `page`，一般不截图，以日志与 `error` 为准。
