...
| `PAGE_LOAD_TIMEOUT_MS`                    | 否                    | `page.goto` 等**整页**导航超时（毫秒），缺省 90000。                                                                                       |
| `PHASE2_FORM_TIMEOUT_MS`                  | 否                    | **已打开页后**单步填表/点击等待（毫秒），缺省 12000；与 `PAGE_LOAD_TIMEOUT_MS` 解耦，避免填邮箱前空等过久。                                  |
...
5. **`apply_outlook_signup_profile`**（`src/apply_signup_profile.py`）  
   - 邮箱 → 下一步 → 密码 → 下一步 → **姓名 + 生日**（`birth_date` 为 `YYYY-MM-DD`）→ 再下一步；单步超时由 `PHASE2_FORM_TIMEOUT_MS` 控制。  
