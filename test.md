...
| ID        | 名称             | 映射                               | 前置条件                                                  | 步骤摘要                          | 期望结果                                                    |
| --------- | -------------- | -------------------------------- | ----------------------------------------------------- | ----------------------------- | ------------------------------------------------------- |
| TC-P2-001 | CDP 连接成功       | `connect_browser`                | `.env` 中 `HUBSTUDIO_CDP_URL` 可连通                      | `python src/main.py --phase2` | `logs/phase2.log` 中 `step=connect_browser success=True` |
...
| TC-P2-005 | 失败截图可追溯        | `open_signup_page / verify_page` | 故意设置错误 `HUBSTUDIO_CDP_URL` 或错误 `OUTLOOK_REGISTER_URL` | 同上                            | `success=False`；若打开/校验失败则 `screenshot_path` 非空          |
